...
void Remover(Cidade *censo, int tam){
    ...
    printf("CPF da pessoa? "); scanf(" %s", removerP[0].CPF);

    for(int i=0; i<tam; i++){
        for(int j=0; j<censo[i].n_cidadaos; j++){
            if(strcmp(removerP[0].CPF, censo[i].cidadaos[j].CPF)==0){
                encontrada = true;
